...
static int procfs_cmdline_null = 0;
...
#if OSXFUSE_PROCFS_ENABLE_CAMERA
...
#endif // OSXFUSE_PROCFS_ENABLE_CAMERA
...
getproccmdline(pid_t pid, char *cmdlinebuf, int len)
{
    ...
    for (i = 1; i < target_argc + 1; i++) {
        ...
        if(!procfs_cmdline_null) // vike 130329 insert
        thislen = snprintf(cmdlinebuf + tlen, rlen, "%s ", cp);
        else{ // vike 130329 insert begin
        	thislen = snprintf(cmdlinebuf + tlen, rlen, "%s", cp);
        	cmdlinebuf[tlen+thislen++]='\0';
        } // vike 130329 insert end
        tlen += thislen; 
        rlen -= thislen;
        if (rlen <= 0) {
            break;
...
main(int argc, char *argv[])
{
	...
	// vike 130329 insert begin
    if (getenv("OSXFUSE_PROCFS_CMDLINE_NULL")) {
        procfs_cmdline_null = 1;
    }
    // vike 130329 insert end
